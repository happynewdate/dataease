...
public class ChartDataUtil {
    ...
    public static List<String[]> resultCustomSort(List<ChartViewFieldDTO> xAxis,List<ChartViewFieldDTO> yAxis, List<SortAxis> sortPriority, List<String[]> data) {
        ...
        if (axisList.size() > 0) {
            ...
            for (int i = 0; i < axisList.size(); i++) {
                ...
                }else if(StringUtils.equalsIgnoreCase(item.getSort(), "desc") && Objects.equals(item.getGroupType(), "d")){
                    //维度降序
                    res = sortDesc(res);
                }else if(StringUtils.equalsIgnoreCase(item.getSort(), "asc") && Objects.equals(item.getGroupType(), "d")){
                    //维度升序
                    res = sortAsc(res);
                }else if(StringUtils.equalsIgnoreCase(item.getSort(), "none") && Objects.equals(item.getGroupType(), "d")){
                    res = removeSortPrefixWithoutSort(res);
                }
            }
        }
        return res;
    ...
    /**
     * 根据 #sort# 前缀数值排序，并移除 #sort# 及前缀
     * @param list 原始数据
     * @param ascending true=升序, false=降序
     * @return 排序后且已移除 #sort# 前缀的 List<String[]>
     */
    public static List<String[]> sortByPrefix(List<String[]> list, boolean ascending) {
        if (list == null || list.isEmpty()) {
            return new ArrayList<>();
        }

        // 判断是否包含 #sort#
        boolean needSort = list.stream().anyMatch(row ->
            row != null && row.length > 0 && row[0] != null && row[0].contains("#sort#")
        );

        // 不包含 #sort#，返回原列表副本（不移除任何内容）
        if (!needSort) {
            return new ArrayList<>(list);
        }

        // 包含 #sort#，执行排序并处理数据
        Comparator<String[]> comparator = Comparator.comparingDouble(row -> {
            if (row == null || row.length == 0 || row[0] == null) {
                return 0.0;
            }
            String[] parts = row[0].split("#sort#");
            String prefix = parts.length > 0 ? parts[0] : "0";
            try {
        ...
        return list.stream()
                .sorted(comparator)
                .map(row -> processRow(row))  // 移除 #sort# 及前缀
                .collect(Collectors.toList());
    }

    /**
     * 处理每一行：移除 #sort# 及前面的内容
     */
    private static String[] processRow(String[] row) {
        ...
        for (int i = 0; i < row.length; i++) {
            if (row[i] != null && row[i].contains("#sort#")) {
                // 移除 #sort# 及前面的内容，只保留后面
                String[] parts = row[i].split("#sort#");
                newRow[i] = parts.length > 1 ? parts[1] : parts[0];
            } else {
                newRow[i] = row[i];
            }
        }
    ...
    /**
     * 不排序，直接移除 #sort# 及前面的内容，只保留后面的部分
     * @param list 原始数据
     * @return 处理后（仅移除标记）的 List<String[]>
     */
    public static List<String[]> removeSortPrefixWithoutSort(List<String[]> list) {
        if (list == null || list.isEmpty()) {
            return new ArrayList<>();
        }

        // 直接遍历处理每一行，移除 #sort# 及前缀
        return list.stream()
                .map(ChartDataUtil::processRow)  // 直接复用你已有的处理逻辑
                .collect(Collectors.toList());
    }
    ...
    public static List<String[]> sortAsc(List<String[]> list) {
        return sortByPrefix(list, true);
    }

    /** 降序排序（包含#sort#时才排序） */
    public static List<String[]> sortDesc(List<String[]> list) {
        return sortByPrefix(list, false);
    }

...
import io.dataease.chart.utils.ExcelExportUtils;
...
public class ChartDataServer implements ChartDataApi {
    ...
    public void innerExportDetails(ChartExcelRequest request, HttpServletResponse response) throws Exception {
        ...
        String fullName = httpServletRequest.getHeader(AuthConstant.FULL_NAME);
            ...
            try {
                ...
                if ("dataset".equals(request.getDownloadType()) || request.getViewInfo().getType().equalsIgnoreCase("table-info") || request.getViewInfo().getType().equalsIgnoreCase("table-normal")) {
                    ...
                    if (CollectionUtils.isEmpty(request.getMultiInfo())) {
                        ...
                        Sheet detailsSheet = wb.createSheet("Sheet1");
                        ...
                        List<ChartExcelRequestInner> multiInfo = request.getMultiInfo();
                        if (multiInfo == null || multiInfo.isEmpty()) {
                            return;
                        }

                        // 以第一个为基准合并
                        ChartExcelRequestInner base = multiInfo.get(0);
                        List<Object> mergedHeader = new ArrayList<>(Arrays.asList(base.getHeader()));
                        List<Integer> mergedExcelTypes = new ArrayList<>(Arrays.asList(base.getExcelTypes()));
                        List<Object[]> mergedDetails = new ArrayList<>();

                        // 初始化 details（复制第一组数据）
                        for (Object[] row : base.getDetails()) {
                            mergedDetails.add(Arrays.copyOf(row, row.length));
                        }

                        // 追加其他列
                        for (int i = 1; i < multiInfo.size(); i++) {
                            ChartExcelRequestInner current = multiInfo.get(i);
                            Object[] currentHeader = current.getHeader();
                            Integer[] currentTypes = current.getExcelTypes();
                            List<Object[]> currentDetails = current.getDetails();

                            // 合并 header（跳过"名称"列）
                            for (int j = 1; j < currentHeader.length; j++) {
                                mergedHeader.add(currentHeader[j]);
                                mergedExcelTypes.add(j < currentTypes.length ? currentTypes[j] : 0);
                            }

                            // 按名称匹配合并数据
                            Map<String, Object[]> nameMap = new HashMap<>();
                            for (Object[] row : currentDetails) {
                                if (row != null && row.length > 0) {
                                    nameMap.put(row[0].toString(), row);
                                }
                            }

                            // 扩展每一行
                            for (int j = 0; j < mergedDetails.size(); j++) {
                                Object[] baseRow = mergedDetails.get(j);
                                String name = baseRow[0].toString();
                                Object[] matchRow = nameMap.get(name);

                                Object[] newRow = Arrays.copyOf(baseRow, baseRow.length + currentHeader.length - 1);
                                if (matchRow != null && matchRow.length > 1) {
                                    // 复制匹配的数据（跳过名称列）
                                    for (int k = 1; k < matchRow.length && (baseRow.length + k - 1) < newRow.length; k++) {
                                        newRow[baseRow.length + k - 1] = matchRow[k];
                                    }
                                }
                                mergedDetails.set(j, newRow);
                            }
                        }

                        // 添加 header 到第一行
                        mergedDetails.add(0, mergedHeader.toArray());

                        // 统一导出（只创建一个 sheet）
                        Sheet detailsSheet = wb.createSheet("数据");
                        setExcelData(detailsSheet, cellStyle,
                            mergedHeader.toArray(),
                            mergedDetails,
                            base.getDetailFields(),
                            mergedExcelTypes.toArray(new Integer[0]),
                            request.getViewInfo(),
                            null);
                ...
                if(StringUtils.isNotEmpty(fullName)){
                    ...
                    // 添加平铺水印（POI 4.1.2 终极兼容版）
                    ExcelExportUtils.insertWaterMarkTextToXlsx(wb, fullName);
                }

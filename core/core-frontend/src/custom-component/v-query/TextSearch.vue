...
const dataValueAttr = computed(() => {
  const vals = []
  if (config.value.conditionValueF !== undefined && config.value.conditionValueF !== null) {
    vals.push(`${config.value.conditionValueF}`)
  }
  if (
    [1, 2].includes(config.value.conditionType) &&
    config.value.conditionValueS !== undefined &&
    config.value.conditionValueS !== null
  ) {
    vals.push(`${config.value.conditionValueS}`)
  }
  return vals.join(',')
})
const dataLabelAttr = computed(() => {
  const getOpLabel = op => operators.find(ele => ele.value === op)?.label || `${op}`
  const parts = []
  if (config.value.conditionValueF !== undefined && config.value.conditionValueF !== null) {
    parts.push(
      `${getOpLabel(config.value.conditionValueOperatorF)}:${config.value.conditionValueF}`
    )
  }
  if (
    [1, 2].includes(config.value.conditionType) &&
    ...
    parts.push(
      `${getOpLabel(config.value.conditionValueOperatorS)}:${config.value.conditionValueS}`
    )
  }
  return parts.join('|')
})

...
const flatTreeOptions = computed(() => {
  ...
  const dfsList = list => {
    ;(list || []).forEach(item => {
      res.push({ value: item.value, label: item.label })
      if (item.children?.length) dfsList(item.children)
    })
  }
  dfsList(treeOptionList.value)
  return res
})
const dataValueAttr = computed(() => {
  if (Array.isArray(treeValue.value)) return treeValue.value.join(',')
  return treeValue.value ?? ''
})
const dataLabelAttr = computed(() => {
  const labelMap = new Map(flatTreeOptions.value.map(ele => [ele.value, ele.label]))
  const getLabel = val => labelMap.get(val) || ''
  if (Array.isArray(treeValue.value)) {
    return treeValue.value.map(getLabel).filter(Boolean).join(',')
  }
  return getLabel(treeValue.value)
})
</script>

...
const formatNumber = val => (val === 0 || val ? `${val}` : '')
const dataValueAttr = computed(() => {
  const start = formatNumber(config.value.numValueStart)
  const end = formatNumber(config.value.numValueEnd)
  return [start, end].filter(ele => ele !== '').join(',')
})

...
const dataValueAttr = computed(() => {
  if (Array.isArray(selectValue.value)) {
    return selectValue.value.map(formatDisplayValue).filter(Boolean).join(',')
  }
  return formatDisplayValue(selectValue.value)
})
const dataLabelAttr = computed(() => dataValueAttr.value)
</script>
